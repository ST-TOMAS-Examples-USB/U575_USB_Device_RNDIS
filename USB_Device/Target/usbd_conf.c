...
/* Private variables ---------------------------------------------------------*/
#ifdef USE_USB_FS
extenr PCD_HandleTypeDef hpcd_USB_OTG_FS;
#endif
#ifdef USE_USB_HS
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  (void)HAL_PCD_Init(&hpcd_USB_OTG_FS);

  (void)HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_FS, 0x80);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x40);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 1, 0x60);
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 2, 0x20);
#endif
  ...
  (void)HAL_PCDEx_SetTxFiFo(&hpcd_USB_HS, 0, 0x40);
